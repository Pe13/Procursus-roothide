...
if which aarch64-apple-darwin-clang; then
    echo "*** an existing toolchain is already in your PATH, please remove it from PATH before running this script"
    exit
fi
...
if [ ! -f ./cctools-port/.build_complete ]; then
    ...
    git clone --recursive --depth 1 https://github.com/tpoechtrager/cctools-port
fi
...
echo '*** All done ***, now you can add
 ...
 to PATH to build Procursus packages ...
'
